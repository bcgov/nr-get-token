...
const permissionHelpers = {
   ...
   */
  checkAcronymPermission: async (token, applicationAcronym) => {
    // Get Acronyms from DB
    const acronyms = await userService.getUserAcronymList(token.sub);

    // Do they have access to the Acronym they are trying to POST
    if (!acronyms.includes(applicationAcronym)) {
      log.verbose('checkAcronymPermission', `User not authorized for acronym ${applicationAcronym}. Token: ${JSON.stringify(token)}`);
