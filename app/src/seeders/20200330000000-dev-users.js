...
module.exports = {
  ...
  down: async (queryInterface, Sequelize) => {
    return queryInterface.sequelize.transaction(async (t) => {
      ...
      const lifecycleHistoryFks =
        await queryInterface.getForeignKeyReferencesForTable(
          'lifecycle_history',
          {}
        );
      const lifecycleFks = await queryInterface.getForeignKeyReferencesForTable(
        'lifecycle',
        {}
      );

      if (
        lifecycleHistoryFks.some(
          (x) => x.constraintName === 'lifecycle_history_userId_fkey'
        )
      ) {
        await queryInterface.removeConstraint(
          'lifecycle_history',
          'lifecycle_history_userId_fkey',
          {
            transaction: t,
          }
        );
      ...
      if (
        lifecycleFks.some(
          (x) => x.constraintName === 'lifecycle_acronymId_fkey'
        )
      ) {
        await queryInterface.removeConstraint(
          'lifecycle',
          'lifecycle_acronymId_fkey',
          {
            transaction: t,
          }
        );
      ...
      return Promise.all([
        queryInterface.bulkDelete(
          ...
          {
            userAcronymId: {
              [Sequelize.Op.in]: userAcronymData.map(
                (entry) => entry.userAcronymId
              ),
            },
          },
          { transaction: t }
