...
const email = {
  ...
  */
  sendConfirmationEmail: async (acronym, ministry, idir, comments, status, nextSteps, to) => {
    try {
      const apiEndpoint = config.get('serviceClient.ches.apiEndpoint');
      const token = await email._getChesToken();
      ...
      const data = {
        ...
        bodyType: 'html',
        contexts: [{
          context: { acronym: acronym, ministry: ministry, idir: idir, comments: comments, status: status, nextSteps: nextSteps },
          to: [to],
        }],
