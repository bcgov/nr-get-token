...
const appConfig = {
  ...
  postAppConfig: async (configForm, publicKey, userId) => {
    ...
    const username = config.get(`serviceClient.webAde.${webadeEnv.toLowerCase()}.username`);
    const password = config.get(`serviceClient.webAde.${webadeEnv.toLowerCase()}.password`);
  ...
  getAppConfigs: async webadeEnv => {
    const env = webadeEnv.toLowerCase();
    const endpoint = config.get(`serviceClient.webAde.${env}.endpoint`);
    const username = config.get(`serviceClient.webAde.${env}.username`);
    const password = config.get(`serviceClient.webAde.${env}.password`);
  ...
  getAppConfig: async (webadeEnv, applicationAcronym) => {
    ...
    const token = await utils.getWebAdeToken(username, password, 'WEBADE-REST', webadeEnv);
    if (!token || token.error) {
      log.error('getAppConfig', 'Unable to acquire access_token');
      throw new Error('Unable to acquire access_token');
    }
